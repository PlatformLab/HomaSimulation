...
stretchVsSize <- read.table("stretchVsTransport_changeUnschedBytes.txt",
    na.strings = "NA",
...
stretchVsSize$UnschedBytes <- factor(stretchVsSize$UnschedBytes)
...
for (rho in unique(tailStretchVsSize$LoadFactor)) {
    ...
    pdf(sprintf("plots/changeUnschedBytes/TailStretchVsUnschedBytes_rho%s.pdf",
        rho), width=40, height=20*length(unique(tailStretchVsSize$WorkLoad)))
...
for (rho in unique(medianStretchVsSize$LoadFactor)) {
    ...
    pdf(sprintf("plots/changeUnschedBytes/MedianStretchVsUnschedBytes_rho%s.pdf",
        rho), width=40, height=20*length(unique(medianStretchVsSize$WorkLoad)))
...
for (rho in unique(avgStretchVsSize$LoadFactor)) {
    ...
    pdf(sprintf("plots/changeUnschedBytes/MeanStretchVsUnschedBytes_rho%s.pdf",
        rho), width=40, height=20*length(unique(avgStretchVsSize$WorkLoad)))

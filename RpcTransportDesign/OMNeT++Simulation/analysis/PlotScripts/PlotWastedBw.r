...
wastedBw <- read.table("wastedBw.txt", na.strings = "NA", col.names=c('transport', 'workload',
    'redundancyFactor', 'loadFactor', 'nominalLoadFactor', 'wastedBw', 'activeWastedBw',
    'totalWastedOversubBw', 'oversubWastedOversubBw'),
    header=TRUE)
...
for (workload in workloads) {
    ...
    plotList[[i]] <- ggplot(wastedBw[wastedBw$workload==workload,]) +
        geom_line(aes(x=loadFactor, y=wastedBw, colour=redundancyFactor), size = 1.5, alpha = 2/3) +
        geom_line(aes(x=loadFactor, y=totalWastedOversubBw, colour=redundancyFactor), size = 1.5, alpha = 2/3) +
        geom_line(data=data.frame(loadFactor=0:100, wastedBw=100:0), aes(x=loadFactor, y=wastedBw), color='magenta', size = 1, alpha = 2/3) +

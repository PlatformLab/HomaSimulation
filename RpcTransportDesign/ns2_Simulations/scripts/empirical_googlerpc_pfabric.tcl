...
puts "Dynamic Flow - Google Search RPC"
...
#############  Agents  #########################
set lambda [expr ($link_rate*$load*1000000000)/($meanFlowSize*8.0/440.8*530.6)]
puts "Arrival: Poisson with inter-arrival [expr 1/$lambda * 1000] ms"
puts "FlowSize: Google Search RPC with mean = $meanFlowSize"
...
for {set j 0} {$j < $S } {incr j} {
	...
	if {$i != $j} {
	    ...
	    $agtagr($i,$j) set_PCarrival_process  [expr $lambda/($S - 1)] "Google_SearchRPC.tcl" [expr 17*$i+1244*$j] [expr 33*$i+4369*$j]
